...
if (isset($_SESSION['id'])) {
    if (filter_input(INPUT_GET, 'delete')) {
        $condition = array("ID_ANNONCE" => filter_input(INPUT_GET, 'delete'));
        if (delete($connection, "ANNONCE", $condition)) {
            $msg = '<div class="alert bg-success text-center">
                       <strong>Succès !</strong> ANNONCE a été bien supprimée !.
                    </div>';
        }
    }
} else {
                ...
                <div class="col-lg-12">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            Liste des annonces
                            <a href="nvlannonce.php" class="btn btn-default pull-right"><svg class="glyph stroked plus sign"><use xlink:href="#stroked-plus-sign"/></svg>Nouvelle</a>
                        </div>
                        <div class="panel-body">
                            <table data-toggle="table" data-url="#" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="name" data-sort-order="desc">
                                <thead>
                                    <tr>
                                        <th class="text-center" data-field="nom" data-sortable="true">Titre</th>
                                        <th class="text-center" data-field="date" data-sortable="true">Date</th>
                                        <th class="text-center" data-field="annonce" data-sortable="true">Annonce</th>
                                        <th class="text-center">ACTION</th>
                                    </tr>
                                </thead>
                                <tbody class="text-center">
                                    <?php
                                    $annonces = read($connection, "ANNONCE", array("*"), array());
                                    foreach ($annonces as $annonce) :
                                        ?>
                                        <tr>
                                            <td><?= $annonce["NOM_ANNONCE"]; ?></td>
                                            <td><?= $annonce["DATE_ANNONCE"]; ?></td>
                                            <td><?= $annonce["TEXTE_ANNONCE"]; ?></td>
                                            <td>
                                                <a href="voirannonce.php?id=<?= $annonce["ID_ANNONCE"]; ?>" class="btn btn-sm btn-default"><i class="glyphicon glyphicon-eye-open"></i></a>
                                                <a href="modannonce.php?id=<?= $annonce["ID_ANNONCE"]; ?>" class="btn btn-sm btn-primary"><i class="glyphicon glyphicon-edit"></i></a>
                                                <a href="#" class="btn btn-sm btn-danger" data-href="annonce.php?delete=<?= $annonce["ID_ANNONCE"]; ?>" data-toggle="modal" data-target="#confirm-delete"><i class="glyphicon glyphicon-trash"></i></a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
